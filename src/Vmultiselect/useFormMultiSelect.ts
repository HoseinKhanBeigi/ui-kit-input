...
function useFormMultiSelect(props: any, emit: InputEmitType) {
  ...
  const is_search = ref<HTMLInputElement>();
  ...
  const pointerPosition = computed(() => {
    return pointer * props.optionHeight;
  });
  const visibleElements = computed(() => {
    return optimizedHeight / props.optionHeight;
  });
  ...
  const valueKeys = computed(() => {
    if (props.trackBy) {
      return internalValue.value.map((element: any) => element[props.trackBy]);
    } else {
  ...
  watch(
    ...
    (newValue) => {
      if (newValue) {
        is_search.value &&
          is_search.value.setAttribute(
            "aria-activedescendant",
            props.id + "-" + pointer.toString()
          );
      }
    }
  );
  ...
  watch(
    ...
    (newValue) => {
      if (newValue) {
        emit("search-change", search, props.id);
      }
    }
  );

  const optionHighlight = (index: any, option: any) => {
    return {
      "multiselect__option--highlight": index === pointer && props.showPointer,
      "multiselect__option--selected": isSelected(option),
    };
  };

  const groupHighlight = (index: any, selectedGroup: any) => {
      ...
      ? [
          "multiselect__option--group",
          {
            "multiselect__option--highlight":
              index === pointer && props.showPointer,
          },
          {
      ...
      if (
        ...
        list.value.scrollTop <=
          pointerPosition.value -
            (visibleElements.value - 1) * props.optionHeight
      ) {
        list.value.scrollTop =
          pointerPosition.value -
          (visibleElements.value - 1) * props.optionHeight;
  ...
  const select = (option: any, key: any) => {
    ...
    if (props.max && props.multiple && internalValue.value.length === props.max)
      return;
  ...
  const wholeGroupDisabled = (group: any) => {
  ...
  function activate() {
    ...
    if (props.searchable) {
      ...
      nextTick(() => is_search.value && is_search.value?.focus());
    } else {
      // this.$el.focus()
    }
    emit("open", props.id);
